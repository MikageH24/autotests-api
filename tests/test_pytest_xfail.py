...
# Маркировка @pytest.mark.xfail используется для пометки тестов, которые ожидаемо проваливаются.
# То есть тесты запускаются и не пропускаются
# Если тест неожиданно проходит, то статус будет XPASS вместо XFAIL
